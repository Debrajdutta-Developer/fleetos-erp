...
void main() {
  group('VehicleComplianceHelper Tests', () {
    ...
    final tExpiredVehicle = VehicleEntity(
      ...
      fuelType: 'diesel',
      odometer: 200.0,
      lastServiceDate: DateTime.now().subtract(
        const Duration(days: 200),
      ), // Overdue
      insuranceExpiry: DateTime.now().subtract(
        const Duration(days: 2),
      ), // Expired
      pucExpiry: DateTime.now().subtract(const Duration(days: 5)), // Expired
      fitnessExpiry: DateTime.now().subtract(
    ...
    test('should identify document warning thresholds correctly', () {
      final warningVehicle = tActiveVehicle.copyWith(
        insuranceExpiry: DateTime.now().add(
          const Duration(days: 10),
        ), // Warning (0-30 days)
        pucExpiry: DateTime.now().add(
