...
void main() {
  group('Trip Business Rules Tests', () {
    ...
    setUp(() {
      ...
      tExpiredPermitVehicle = tValidVehicle.copyWith(
        id: 'v_exp_permit',
      );
      // Explicitly set the mock permit to yesterday
      VehiclePermitValidator.setPermitExpiry('v_exp_permit', now.subtract(const Duration(days: 1)));
      // Set valid vehicle permit to next year
      VehiclePermitValidator.setPermitExpiry('v_valid', now.add(const Duration(days: 365)));
    });
    ...
    test('should block trip creation if insurance is expired', () async {
      ...
      final result = await controller.saveTrip(trip, selectedVehicle: tExpiredInsuranceVehicle);
    ...
    test('should block trip creation if PUC is expired', () async {
      ...
      final result = await controller.saveTrip(trip, selectedVehicle: tExpiredPucVehicle);
      final formState = container.read(tripFormControllerProvider);
    ...
    test('should block trip creation if fitness is expired', () async {
      ...
      final result = await controller.saveTrip(trip, selectedVehicle: tExpiredFitnessVehicle);
      final formState = container.read(tripFormControllerProvider);

      expect(result, false);
      expect(formState.errorMessage, contains('fitness certificate is expired'));
      expect(repository.trips.length, 0);
    });
    ...
    test('should block trip creation if road permit is expired', () async {
      ...
      final result = await controller.saveTrip(trip, selectedVehicle: tExpiredPermitVehicle);
      final formState = container.read(tripFormControllerProvider);
    ...
    test('should block trip creation if selected vehicle is already on another active trip', () async {
      ...
      final result = await controller.saveTrip(trip, selectedVehicle: tValidVehicle);
      final formState = container.read(tripFormControllerProvider);

      expect(result, false);
      expect(formState.errorMessage, contains('vehicle is already assigned to another active trip'));
      expect(repository.trips.length, 1);
    });

    test('should block trip creation if selected driver is already on another active trip', () async {
      final existingActiveTrip = TripEntity(
        id: 't_active',
        companyId: 'comp_1',
        vehicleId: 'v_another',
      ...
      expect(result, false);
      expect(formState.errorMessage, contains('driver is already assigned to another active trip'));
      expect(repository.trips.length, 1);
    });

    test('should succeed saving trip if resources are valid and unassigned', () async {
      final repository = MockTripRepository(trips: []);
      final container = ProviderContainer(
        overrides: [
          currentUserProvider.overrideWith((ref) => UserEntity(
