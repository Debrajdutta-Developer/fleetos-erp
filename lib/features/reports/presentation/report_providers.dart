  ...
  try {
    ...
    switch (type) {
      case 'financial_revenue':
        ...
        final totalInvoiced = filteredInvoices.fold<double>(0.0, (acc, i) => acc + i.grandTotal);
        final totalCollected = filteredInvoices.fold<double>(0.0, (acc, i) => acc + i.amountPaid);
        final totalOutstanding = filteredInvoices.fold<double>(0.0, (acc, i) => acc + i.outstandingAmount);
      ...
      case 'financial_expense':
        ...
        final totalExp = filteredTxs.fold<double>(0.0, (acc, t) => acc + t.amount);
        final fuelExp = filteredTxs.where((t) => t.category == 'diesel').fold<double>(0.0, (acc, t) => acc + t.amount);
        final maintExp = filteredTxs.where((t) => t.category == 'repair' || t.category == 'tyre').fold<double>(0.0, (acc, t) => acc + t.amount);
        final salaryExp = filteredTxs.where((t) => t.category == 'driver_salary' || t.category == 'advance_salary').fold<double>(0.0, (acc, t) => acc + t.amount);
      ...
      case 'financial_profit_loss':
        ...
        final invoiceRevenue = invoices.where((i) => i.status != 'draft' && i.status != 'cancelled' && inDateRange(i.issueDate)).fold<double>(0.0, (acc, i) => acc + i.grandTotal);
        final financeIncome = filteredIncomes.fold<double>(0.0, (acc, t) => acc + t.amount);
        final totalIncome = financeIncome > 0.0 ? financeIncome : invoiceRevenue;

        final totalExpense = filteredExpenses.fold<double>(0.0, (acc, t) => acc + t.amount);
      ...
      case 'financial_cash_flow':
        ...
        final inflows = filteredPayments.fold<double>(0.0, (acc, p) => acc + p.amount);
        final outflows = filteredExpenses.fold<double>(0.0, (acc, t) => acc + t.amount);
      ...
      case 'financial_outstanding_receivables':
        ...
        final totalOut = activeInvoices.fold<double>(0.0, (acc, i) => acc + i.outstandingAmount);
        
        final now = DateTime.now();
        final aging30 = activeInvoices.where((i) => now.difference(i.issueDate).inDays <= 30).fold<double>(0.0, (acc, i) => acc + i.outstandingAmount);
        final aging60 = activeInvoices.where((i) => now.difference(i.issueDate).inDays > 30 && now.difference(i.issueDate).inDays <= 60).fold<double>(0.0, (acc, i) => acc + i.outstandingAmount);
        final aging90 = activeInvoices.where((i) => now.difference(i.issueDate).inDays > 60).fold<double>(0.0, (acc, i) => acc + i.outstandingAmount);
      ...
      case 'financial_customer_ledger':
        ...
        final customer = customers.firstWhereOrNull((c) => c.id == custId) ?? CustomerEntity(id: custId, name: 'Unknown Customer', contactName: '', email: '', phone: '', address: '', createdAt: DateTime.now(), updatedAt: DateTime.now());
        final custInvoices = invoices.where((i) => i.customerId == custId && inDateRange(i.issueDate)).toList();
        final totalInv = custInvoices.fold<double>(0.0, (acc, i) => acc + i.grandTotal);
        final totalPaid = custInvoices.fold<double>(0.0, (acc, i) => acc + i.amountPaid);
      ...
      case 'financial_driver_expense':
        ...
        final totalSalaries = filteredTxs.where((t) => t.category == 'driver_salary').fold<double>(0.0, (acc, t) => acc + t.amount);
        final totalAdvances = filteredTxs.where((t) => t.category == 'advance_salary').fold<double>(0.0, (acc, t) => acc + t.amount);
      ...
      case 'financial_vehicle_expense':
        ...
        final totalFuel = filteredTxs.where((t) => t.category == 'diesel').fold<double>(0.0, (acc, t) => acc + t.amount);
        final totalMaint = filteredTxs.where((t) => t.category == 'repair' || t.category == 'tyre').fold<double>(0.0, (acc, t) => acc + t.amount);
        final totalMisc = filteredTxs.where((t) => t.category == 'toll' || t.category == 'insurance' || t.category == 'miscellaneous').fold<double>(0.0, (acc, t) => acc + t.amount);
      ...
      case 'fleet_trip_summary':
        ...
        rows = filteredTrips.map((t) => {
          'Trip ID': t.id,
          'Trip Number': t.id,
          'Driver': drivers.firstWhereOrNull((d) => d.id == t.driverId)?.fullName ?? 'Unknown',
          'Vehicle': vehicles.firstWhereOrNull((v) => v.id == t.vehicleId)?.licensePlate ?? 'Unknown',
      ...
      case 'fleet_driver_performance':
        final avgSafety = drivers.isEmpty ? 0.0 : drivers.fold<double>(0.0, (acc, d) => acc + d.safetyScore) / drivers.length;
      ...
      case 'fleet_fuel_consumption':
        ...
        final totalLiters = filteredFuels.fold<double>(0.0, (acc, f) => acc + f.fuelQty);
        final totalCost = filteredFuels.fold<double>(0.0, (acc, f) => acc + f.amount);
      ...
      case 'fleet_maintenance_cost':
        ...
        final totalCost = filteredMaints.fold<double>(0.0, (acc, m) => acc + m.cost);
        final preventative = filteredMaints.where((m) => m.type == 'preventative').fold<double>(0.0, (acc, m) => acc + m.cost);
        final corrective = filteredMaints.where((m) => m.type == 'corrective').fold<double>(0.0, (acc, m) => acc + m.cost);
      ...
      case 'fleet_inventory_usage':
        ...
        final totalPartsUsed = maintParts.fold<int>(0, (acc, m) => acc + (m.partQuantity ?? 0));
      ...
      case 'customer_revenue':
        ...
        final totalRev = filteredInvoices.fold<double>(0.0, (acc, i) => acc + i.grandTotal);
      ...
      case 'customer_outstanding':
        ...
        final totalOut = unpaidInvoices.fold<double>(0.0, (acc, i) => acc + i.outstandingAmount);
      ...
      case 'customer_payment_history':
        ...
        final totalInflows = filteredPayments.fold<double>(0.0, (acc, p) => acc + p.amount);
      ...
      case 'customer_contract_summary':
        ...
        for (final c in filteredContracts) {
          final cust = customers.firstWhereOrNull((cust) => cust.id == c.customerId);
        ...
        rows = filteredContracts.map((c) {
          ...
          return {
            ...
            'Freight Billing Details': '\$${c.defaultFreightRate.toStringAsFixed(2)} (Default)',
